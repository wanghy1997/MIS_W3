...
class BTCV(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(self._base_dir, 'btcv_h5', '{}.h5'.format(image_name))
...
class BTCV_fast(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            'orig_shape': np.asarray(image.shape, dtype=np.int32),
        }
        if self.transform:
            sample = self.transform(sample)
        return sample
...
class MMWHS(Dataset):
    ...
    def __init__(self, image_list, base_dir=None, transform=None):
        ...
        for i in range(len(self.image_list)):
            ...
            image_path = os.path.join(self._base_dir, 'npy', 'ct_train_{}_image.npy'.format(image_name))
            label_path = os.path.join(self._base_dir, 'npy', 'ct_train_{}_label.npy'.format(image_name))
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        sample['crop_bbox'] = np.asarray(
            [w1, w1 + self.output_size[0], h1, h1 + self.output_size[1], d1, d1 + self.output_size[2]],
            dtype=np.int32,
        )
        sample['pad_width'] = np.asarray([pw, pw, ph, ph, pd, pd], dtype=np.int32)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        output = {'image': torch.from_numpy(image), 'label': torch.from_numpy(label).long()}
        if 'onehot_label' in sample:
            output['onehot_label'] = torch.from_numpy(sample['onehot_label']).long()
        for key in ['case_name', 'case_index', 'is_labeled', 'orig_shape', 'crop_bbox', 'pad_width']:
            if key not in sample:
                continue
            value = sample[key]
            if isinstance(value, np.ndarray):
                output[key] = torch.from_numpy(value).long()
            elif isinstance(value, (int, np.integer)):
                output[key] = torch.tensor(value).long()
            else:
                output[key] = value
        return output
...
class Synapse_fast(Dataset):
    """
    Synapse 3D dataset from .h5 files.
    Expected:
      base_dir/CASE_ID.h5
      keys: image, label
    """
    ...
    def __init__(self, labeled_list, unlabeled_list, base_dir=None, transform=None):
        ...
        self.images_l = []
        self.labels_l = []
        for i, image_name in enumerate(self.labeled_list):
            image_path = os.path.join(self._base_dir, f"{image_name}.h5")
            with h5py.File(image_path, "r") as h5f:
                image = h5f["image"][:]
                label = h5f["label"][:]
            self.images_l.append(image)
            self.labels_l.append(label)
            print(f"Loading {i:2d}-th labeled sample from {image_path}")

        self.images_u = []
        self.labels_u = []
        for i, image_name in enumerate(self.unlabeled_list):
            image_path = os.path.join(self._base_dir, f"{image_name}.h5")
            with h5py.File(image_path, "r") as h5f:
                image = h5f["image"][:]
                label = h5f["label"][:]
            self.images_u.append(image)
            self.labels_u.append(label)
            print(f"Loading {i:2d}-th unlabeled sample from {image_path}")
    ...
    def __getitem__(self, idx):
        if idx < len(self.unlabeled_list) * 2:
            idx = idx % len(self.labeled_list)
            image = self.images_l[idx]
            label = self.labels_l[idx]
            case_name = self.labeled_list[idx]
            is_labeled = 1
        else:
            idx = idx % len(self.unlabeled_list)
            image = self.images_u[idx]
            label = self.labels_u[idx]
            case_name = self.unlabeled_list[idx]
            is_labeled = 0
...
class Synapse_fast_npy(Dataset):
    """
    Synapse 3D dataset from .npy files.
    Expected:
      base_dir/CASE_ID_image.npy
      base_dir/CASE_ID_label.npy
    """
    ...
    def __init__(self, labeled_list, unlabeled_list, base_dir=None, transform=None):
        ...
        self.labels_l = []
        for i, image_name in enumerate(self.labeled_list):
            image_path = os.path.join(self._base_dir, f"{image_name}_image.npy")
            label_path = os.path.join(self._base_dir, f"{image_name}_label.npy")
            image = np.load(image_path)
            label = np.load(label_path)
            self.images_l.append(image)
            self.labels_l.append(label)
