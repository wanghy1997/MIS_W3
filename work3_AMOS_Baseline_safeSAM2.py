...
def log_validation_summary(iter_num, dice_avg, dice_all):
    ...
    parts.extend(f"{name}: {score:.4f}" for name, score in zip(CLASS_NAMES, dice_all[:len(CLASS_NAMES)]))
    logging.info(', '.join(parts))


def log_metric_summary(final_mean, final_std, class_mean, class_std):
    logging.info(
        'Final Average DSC:{:.4f}±{:.4f}, HD95: {:.4f}±{:.4f}, JI: {:.4f}±{:.4f}, ASD: {:.4f}±{:.4f}'.format(
            final_mean[0], final_std[0],
            final_mean[1], final_std[1],
            final_mean[2], final_std[2],
            final_mean[3], final_std[3],
        )
    )
    log_str = ''
    for idx, organ in enumerate(CLASS_NAMES):
        if idx < class_mean.shape[1]:
            log_str += '\n{}: {:.4f}±{:.4f}, {:.4f}±{:.4f}, {:.4f}±{:.4f}, {:.4f}±{:.4f},'.format(
                organ,
                class_mean[0][idx], class_std[0][idx],
                class_mean[1][idx], class_std[1][idx],
                class_mean[2][idx], class_std[2][idx],
                class_mean[3][idx], class_std[3][idx],
            )
...
def train(labeled_list, unlabeled_list, eval_list, fold_id=1):
    ...
    if args.resume_path:
        ...
        if (not loaded_training_state) and iter_num == 0:
            model.eval()
            dice_all, _, _ = test_amos.validation_all_case_fast(
                model,
                num_classes=num_classes,
                base_dir=train_data_path,
            ...
            logging.info('Resume checkpoint validation before further training: average DSC {:.4f}', float(dice_all.mean()))
            model.train()
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            part_and_rec = cube_utils.get_part_and_rec_ind(
                volume_shape=volume_batch.shape,
                nb_cubes=nb_cubes,
                nb_chnls=16,
            )
            cube_part_ind, cube_rec_ind = part_and_rec[:2]
            img_cross_mix = torch.gather(volume_batch.view(bs, c, w, h, d), dim=0, index=cube_part_ind)
            img_cross_mix = img_cross_mix.view(bs, c, w, h, d)

            _, embedding = model(img_cross_mix)
            ...
            with torch.no_grad():
                ...
                if medsam2_teacher is not None and iter_num >= args.medsam2_warmup:
                ...
                _, pred_class_teacher = torch.max(teacher_probs, dim=1)
            ...
            if feat_list is not None:
                embed_list = []
                for i in range(bs):
                    _, embed_tmp = model.forward_decoder(feat_list[i])
            ...
            with torch.no_grad():
                if iter_num > 100 and feat_list is not None:
                    current_organ_dist = dist_logger.get_class_dist().cuda()
                    ...
                    _, pr_class = torch.max(unlab_pl_mix_soft.detach(), dim=1)
                    dist_logger.append_class_list(pr_class.view(-1, 1))
                elif feat_list is not None:
                    _, pr_class = torch.max(teacher_probs.detach(), dim=1)
                    dist_logger.append_class_list(pr_class.view(-1, 1))
            ...
            if medsam2_patch_probs is not None and medsam2_trusted_mask is not None:
                ...
                if medsam2_mask.sum().item() >= max(1, int(args.medsam2_safe_min_trusted_voxels)):
                    ...
                    medsam2_soft_loss = (
                        masked_kl_loss(outputs[labeled_bs:], medsam2_patch_probs, medsam2_mask, temperature=args.medsam2_distill_temperature)
                        + masked_kl_loss(outputs_unmix[labeled_bs:], medsam2_patch_probs, medsam2_mask, temperature=args.medsam2_distill_temperature)
                    ) / 2.0
            ...
            if iter_num % 100 == 0:
                logging.info(
                    'Fold {}, iteration {}: loss: {:.4f}, lr: {:.6f}, cons_dist: {:.4f}, loss_weight: {:4f}, loss_loc: {:.4f}, medsam2_cov: {:.4f}, medsam2_trust: {:.4f}, medsam2_agree: {:.4f}, medsam2_blend: {:.4f}, medsam2_hard: {:.4f}, medsam2_soft: {:.4f}'.format(
                        fold_id, iter_num, loss, lr_, consistency_loss, consistency_weight, 0.1 * loc_loss,
                        medsam2_coverage, medsam2_trusted_ratio, medsam2_agreement_ratio, medsam2_blend_ratio,
                        medsam2_hard_loss, medsam2_soft_loss,
                    )
                )

            if iter_num % 1000 == 0:
                model.eval()
                dice_all, _, metric_all_cases = test_amos.validation_all_case_fast(
                    model,
                    num_classes=num_classes,
                    base_dir=train_data_path,
                    image_list=eval_list,
...
if __name__ == '__main__':
    ...
    final_mean = np.mean(case_level_mean, axis=0)
    final_std = np.std(case_level_mean, axis=0)
