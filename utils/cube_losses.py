...
def unmix_tensor(patch_list, unmix_shape):
    """
    拼接 patch，但不是重叠拼接，而是直接按位置拼接，还是盲拼
    """
...
def cube_location_loss(model, loc_list, patch_list, idx, do_contrast=False):
    ...
    for i in range(bs):
        ...
        # print(i, len(feat_embed_patch), feat_embed_patch[-1].shape)
...
def cube_location_loss_GA(model, loc_list, patch_list, idx, labeled_bs=2, cube_size=32):
...
def get_mix_pl_baseline(ema_model, unlabeled_feat_list, text_embedding, ori_shape, unlabeled_bs=2):
...
def get_mix_pl(ema_model, unlabeled_feat_list, _, ori_shape, unlabeled_bs=2):
    ...
    with torch.no_grad():
        ...
        for i in range(unlabeled_bs):
            ...
            embedding_tmp = ema_model.forward_decoder(unlabeled_feat_list[i]).detach()
            pred_tmp = ema_model.out_(embedding_tmp)
...
def get_mix_pl_GA(ema_model, feat_list, ori_shape, unlabeled_bs=2):
