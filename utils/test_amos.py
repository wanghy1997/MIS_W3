...
def validation_all_case(model, num_classes, base_dir, image_list,
                        patch_size=(96, 96, 96),
    ...
    for case_idx in loader:
        image_path = base_dir + '/npy/{}_image.npy'.format(case_idx)
        ...
        # === 原始预测（与 image.npy 同 size） ===
        prediction_raw, score_map = test_single_case_fast(
            model, image, stride_xy, stride_z,
        ...
        if save_nii_dir is not None:
            ...
            np.save(save_npy_path, prediction_raw.astype(np.uint8))

        # ================== 以下保持你原来的评估流程 ==================
        prediction = torch.FloatTensor(prediction_raw).unsqueeze(0).unsqueeze(0)

        # 对齐 DHC 尺寸（仅用于评估）
        prediction = F.interpolate(
            prediction, size=(160, 160, 80), mode='nearest'
        ).int()
        ...
        gt_mask = F.interpolate(
            gt_mask, size=(160, 160, 80), mode='nearest'
        ).int()
        ...
        if np.sum(prediction) == 0:
            ...
            for i in range(1, num_classes):
                case_metric[:, i - 1] = cal_metric(
                    prediction == i, gt_mask == i
                )

        total_metric.append(np.expand_dims(case_metric, axis=0))

    all_metric = np.concatenate(total_metric, axis=0)
    avg_dice = np.mean(all_metric, axis=0)[0]
    std_dice = np.std(all_metric, axis=0)[0]
