...
from utils_ori import metrics, ramps, test_util, cube_losses, cube_utils
...
parser.add_argument('--root_path', type=str, default='/data/why/Datasets/Synapse_data/npy', help='Name of Dataset')
...
def infer_full_volume_probs(model, image_np, patch_size, stride_xy, stride_z, num_classes):
    ...
    model.eval()
    try:
        _, score_map = test_util.test_single_case_fast(
            model,
            image_np,
            stride_xy=stride_xy,
            stride_z=stride_z,
...
def read_list(split):
    ids_list = np.loadtxt(
        os.path.join('/data/why/Datasets/Synapse_data/split_txts/', f'{split}.txt'),
        dtype=str
    ).tolist()
...
if __name__ == "__main__":
    ...
    model.eval()
    _, _, metric_final = test_util.validation_all_case(model, num_classes=num_classes, base_dir=train_data_path, image_list=test_list, patch_size=patch_size, stride_xy=32, stride_z=16)
