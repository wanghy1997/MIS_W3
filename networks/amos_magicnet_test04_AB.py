...
from networks.semantic_extraction_2 import MultiScaleFusion3D
from networks.Ratoty import AnatomicalSpaceAttention
...
class VNet_Magic_CLIP_2p_Contrast(nn.Module):
    def __init__(self, n_channels=1, n_classes=16, cube_size=32, patch_size=96, n_filters=16,
        ...
        if n_classes == 16: 
            ...
            ckpt = torch.load("/home/why/TAK-Semi-main/CLIP/biomedbert_text_embeddings_16_ours_0.pt")  # 消融实验
            # self.text_embedding = ckpt["embeddings"].float()   # [32, 512]
            self.text_embedding = self.text_embedding * 0.0001
            ...
            ckpt = torch.load("/home/why/TAK-Semi-main/CLIP/biomedbert_text_embeddings_14_ours_0.pt")
        ...
        self.attn_x5 = AnatomicalSpaceAttention(visual_dim=256, text_dim=512)
    ...
    def forward(self, input, output_embedding=False):
        features, features_embedding_list, text_embedding_list, clip_embedding, sem_pack = self.forward_encoder(
