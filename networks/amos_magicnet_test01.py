...
class VNet_Magic_CLIP_2p_Contrast(nn.Module):
    def __init__(self, n_channels=1, n_classes=16, cube_size=32, patch_size=96, n_filters=16,
        ...
        # # CLIP
        # self.text_embedding = torch.load('/home/why/TAK-Semi-main/CLIP/amos_embedding_shape_location.pth', weights_only=True).float()
        ...
        if n_classes == 16: 
            ...
            self.text_embedding = ckpt["embeddings"].float().detach()    # [32, 512]
            ...
            self.text_embedding = ckpt["embeddings"].float().detach()    # [28, 512]
            text_embedding_dim = self.text_embedding.shape[-1]
        ...
        self.hyper_param_adapter = HyperbolicParamAdapter(
            ...
            curvature=0.1  # 第一次用的 0.001 现在改为 0.01
        )
    ...
    def forward_encoder(self, x, output_embedding=False):
        ...
        # if x5.shape[2] == 6:
        #     x5 = self.doublespace(x5)  # 双空间结构感知模块
        ...
        clip_embedding = self.text_to_vision(torch.cat([text_embedding_256[:self.num_classes],
                                                        text_embedding_256[self.num_classes:]], dim=1))  # [16, 512]
