...
class TwoWayTransformer(nn.Module):
    def __init__(
        ...
        self.Space_Adapter = Adapter(embedding_dim)  # with skip connection
        self.scale = 0.5
    ...
    def forward(
        ...
        attn_out = self.Space_Adapter(attn_out)
        queries = queries + attn_out
        queries = self.norm_final_attn(queries)
        queries = self.Space_Adapter(queries)
        return queries, keys
...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # MLP block
        mlp_out = self.mlp(queries) + self.scale * self.MLP_Adapter(queries)
