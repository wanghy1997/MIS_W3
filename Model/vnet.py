...
class VNet(nn.Module):
    ...
    def forward(self, input, turnoff_drop=False):
        ...
        if turnoff_drop:
            self.has_dropout = has_dropout
        return out, features[4]
        # x5 = self.encoder(input)
        # return x5
...
class VNet_DTC(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        ...
        self.block_nine = convBlock(1, n_filters, n_filters, normalization=normalization)
        self.out_conv = nn.Conv2d(n_filters, n_classes, 1, padding=0)
        self.out_conv2 = nn.Conv2d(n_filters, n_classes, 1, padding=0)
        self.tanh = nn.Tanh()
    ...
    def encoder(self, input):
        ...
        x5 = self.block_five(x4_dw)
        # x5 = F.dropout2d(x5, p=0.5, training=True)
        if self.has_dropout:
            x5 = self.dropout(x5)
    ...
    def decoder(self, features):
        ...
        x8_up = x8_up + x1
        x9 = self.block_nine(x8_up)
        # x9 = F.dropout2d(x9, p=0.5, training=True)
        if self.has_dropout:
            x9 = self.dropout(x9)
        out = self.out_conv(x9)
