...
from utils import ramps, test_amos, cube_losses, cube_utils, test_util
...
parser.add_argument('--dataset_name', type=str, default='FLARE', help='dataset_name')
parser.add_argument('--data_path', type=str, default='/data/why/Datasets/FLARE/', help='data_path')
...
def train(labeled_list, unlabeled_list, eval_list):
    ...
    db_train = FLARE_fast(labeled_list, unlabeled_list,
    ...
    for epoch_num in tqdm(range(args.max_epoch + 1), dynamic_ncols=True, position=0):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with autocast(dtype=torch.float16):
                ...
                if args.sem_align_w > 0 and labeled_bs > 0:  # ⭐️计算语义对齐损失
        ...
        if epoch_num % 5 == 0:
            model.eval()
            dice_all, std_all, metric_all_cases = test_util.validation_all_case_flare(model,
            ...
            logging.info('iteration {}, '
                             ...
                             'portal vein and splenic vein: {:.4f}, '
                             'pancreas: {:.4f}, '
...
if __name__ == "__main__":
    ...
    # save_best_path = '/data/why/logs//FLARE_Ours_50_ours/epoch_10_dice_0.2923_best.pth'
    model = create_model(n_classes=num_classes, cube_size=cube_size, patch_size=patch_size[0])
    model.module.load_state_dict(torch.load(save_best_path, weights_only=True))
    model.eval()
    _, _, metric_final = test_util.validation_all_case_flare(model, num_classes=num_classes, base_dir=train_data_path,
