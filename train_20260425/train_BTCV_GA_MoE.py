...
from networks.magicnet_MoE import VNet_Magic
...
parser.add_argument('--exp', type=str, default='MoE', help='exp_name')
...
snapshot_path = args.log_path + "/MoE_{}_{}_{}_{}".format(args.dataset_name, args.exp, args.label_num, args.remark)
...
def train(labeled_list, unlabeled_list):
    ...
    for epoch_num in tqdm(range(args.max_epoch + 1), dynamic_ncols=True, position=0):
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            cube_part_ind, cube_rec_ind = cube_utils.get_part_and_rec_ind_GA(volume_shape=volume_batch.shape,
                                                                          nb_cubes=nb_cubes,
                                                                          nb_chnls=16)
            ...
            if loc_list is not None:
                loc_loss, feat_list = cube_losses.cube_location_loss_GA(model, loc_list, patch_list, idx, labeled_bs, cube_size=cube_size)
            ...
            with torch.no_grad():
                ...
                # if iter_num > 10:
                    # print('Get organ-class distribution')
...
#  CUDA_VISIBLE_DEVICES=0 python train_BTCV_GA_MoE.py --label_num 10 --remark "GA_MoE"
# 