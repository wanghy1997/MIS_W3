...
from loss_amos import GADice, GACE, ContrastiveLoss, build_cam, refine_cam_with_affinity_3d
from dataloaders.dataset import *
from networks.amos_magicnet_test03 import *
    ...
    for epoch_num in tqdm(range(args.max_epoch + 1), dynamic_ncols=True, position=0):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with autocast(dtype=torch.float16):
                ...
                with torch.no_grad():
                    ema_output, fea_list, _, text_affinity, text_embedding = ema_model(ema_inputs, True, True)  # 仅未标注数据送入ema模型，得到预测结果和特征
                    # fea_list torch.Size([2, 64, 24, 24, 24]) torch.Size([2, 128, 12, 12, 12]) torch.Size([2, 256, 6, 6, 6])
                    unlab_pl_soft = F.softmax(ema_output, dim=1)
                    conf, teacher_pl = torch.max(unlab_pl_soft, dim=1)  # [B, D, H, W]
                    entropy_map = -torch.sum(unlab_pl_soft * torch.log(unlab_pl_soft + 1e-6), dim=1, keepdim=True)
                    uncertain_mask = entropy_map > entropy_threshold
                    """ 方式 1 """
                    E_cls = text_embedding[:num_classes]      # [num_classes,256]  类别语义
                    # E_loc = text_embedding[num_classes:2*num_classes]   # [num_classes,256]  空间语义
                    A_cls = text_affinity[:num_classes]       # [num_classes,256,256]
                    # A_loc = text_affinity[num_classes:2*num_classes]    # [num_classes,256,256]
                    """ 方式 2 """
                    # beta = 0.3  # 建议 0.1~0.3 起步
                    # E_fused = F.normalize(E_cls + beta * E_loc, dim=1)  # [num_classes,256]
                    # A_fused = 0.5 * (A_cls + A_loc)                     # [num_classes,256,256]
                    cam_region = build_cam(
                        feat_3d = fea_list[2],
                        clip_embedding = E_cls,
                        attn_pred = A_cls
                    )
                    # ✅ ViT-agnostic refinement: 用 3D 特征自己算 affinity，然后扩散 CAM
                    cam_region_ref = refine_cam_with_affinity_3d(
                        cam=cam_region,          # [B,K,6,6,6]
                        feat_3d=fea_list[2],      # 用它计算 [B,N,N] affinity（N=216，非常轻）
                        affinity=None,            # 也可以你自己传 affinity
                        class_mask=None,          # 如果你后面想加“类存在性”，再传 [B,K]
                        num_iter=4,
                        alpha=0.7,
                        local_radius=2,           # 建议先开局部，避免过扩散
                        seed_thre=0.7,
                        keep_seeds=True
                    )
                    # cam_region_ref: [B, K, 6, 6, 6]
                    cam_region_ref_up = F.interpolate(
                        cam_region_ref,
                        size=unlab_pl_soft.shape[2:],  # (96,96,96)
                        mode="trilinear",
                        align_corners=False
                    )
                    # -> [B, K, 96, 96, 96]
                    cam_pl = torch.argmax(cam_region_ref_up, dim=1)  # [B,96,96,96]
                    pseudo_label = teacher_pl.clone()
                    mask = uncertain_mask.squeeze(1)  # [B,96,96,96]
                    pseudo_label[mask] = cam_pl[mask]
                    pseudo_label = pseudo_label.unsqueeze(1).long()  # [B,1,96,96,96]
                    # [2, 1, 96, 96, 96]
...
if __name__ == "__main__":
    ...
    # save_best_path = "/home/why/TAK-Semi-main/TAK_logs/AMOS_MagicNet_GA_TAK_10_fuxian/epoch_140_dice_0.5437_best.pth"
