...
from networks.amos_magicnet_test01 import *
...
def create_model(n_classes=16, cube_size=32, patch_size=96,
                 ema=False, local_rank=0, use_ddp=True):

    model = VNet_Magic_CLIP_2p_Contrast(
        n_channels=1,
        n_classes=n_classes,
        cube_size=cube_size,
        patch_size=patch_size,
    ...
    if ema:
        for p in model.parameters():
            p.requires_grad_(False)   # 比 detach_ 更干净

    if use_ddp and (not ema):
        model = DDP(
            model,
