...
if "--use_medsam2" not in argv:
    argv += ["--use_medsam2", "0"]
if "--exp" not in argv:
    argv += ["--exp", "GA_base_fromscratch"]
sys.argv = ["work3_Synapse_Baseline.py", *argv]
import work3_Synapse_Baseline as base
...
def main():
    ...
    model.eval()
    _, _, metric_final = base.test_util.validation_all_case(
        model,
