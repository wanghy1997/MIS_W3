...
def train(labeled_list, unlabeled_list):
    ...
    handler, sh = config_log(snapshot_path, 'train', append=bool(args.resume_path))
    ...
    writer = SummaryWriter(snapshot_path, purge_step=iter_num if args.resume_path else None)
    ...
    for epoch_num in tqdm(range(args.max_epoch + 1), dynamic_ncols=True, position=0):
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # if iter_num >= max_iterations:
                ...
                save_training_state(
                    os.path.join(snapshot_path, 'latest_training_state.pth'),
                    model,
                    ema_model,
                    optimizer,
                    iter_num,
...
if __name__ == "__main__":
    import setproctitle
    setproctitle.setproctitle(f'BTCV{args.label_num}_{args.remark}')
    ...
    # save_best_path = '/data/why/logs/GA_BTCV_GA_50_base0326/iter_23000_dice_0.762768_best.pth'
...
# CUDA_VISIBLE_DEVICES=0 python train_BTCV_GA.py --seed 1337 --label_num 50 --remark "base0326"
