...
from loss_amos import GADice, GACE, ContrastiveLoss_AB, AnatomyWeightScheduler
...
parser.add_argument('--exp', type=str, default='AB', help='exp_name')
parser.add_argument('--remark', type=str, default='AB', help='remark')
...
parser.add_argument('--max_epoch', type=int, default=25000, help='maximum epoch number to train')
    ...
    for epoch_num in tqdm(range(args.max_epoch + 1), dynamic_ncols=True, position=0):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with autocast(dtype=torch.float16):
                ...
                # outputs_unmix = model.module.forward_prediction_head(mix_features_rec,clip_embedding,embedding_rec)
                ...
                with torch.no_grad():
                    ...
                    ema_v_raw, _, ema_T_class = ema_sem_pack
                ...
                # pred_all_unmix = model.module.forward_prediction_head(feat_list_unmix,clip_embedding,embed_all_unmix)
                ...
                with torch.no_grad():
                    ...
                    if epoch_num > args.start_contrast_epoch-1:
                    # if epoch_num > 10:
...
if __name__ == "__main__":
    import setproctitle
    setproctitle.setproctitle(f'{args.dataset_name}{args.label_num}_{args.remark}')
    ...
    # save_best_path = "/data/why/logs/MMWHS_Ours_10_from_envl/epoch_20600_dice_0.8120_best.pth"
