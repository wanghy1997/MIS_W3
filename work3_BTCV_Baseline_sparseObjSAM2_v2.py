...
parser.add_argument('--exp', type=str, default='SAM2SSL_sparseObj_v2', help='exp_name')
...
parser.add_argument('--medsam2_direction_merge', type=str, default='avg', choices=['avg', 'max'], help='merge strategy for forward/reverse SAM2 propagation')
parser.add_argument('--medsam2_multi_class_ratio', type=float, default=0.35, help='minimum confident-voxel ratio for keeping lower-ranked classes')
parser.add_argument('--medsam2_adaptive_blend', type=int, default=1, help='adaptively scale trusted SAM2 teacher blending')
parser.add_argument('--medsam2_blend_min_trust', type=float, default=0.005, help='minimum trusted ratio before enabling adaptive teacher blending')
parser.add_argument('--medsam2_blend_min_agree', type=float, default=0.02, help='minimum agreement ratio before enabling adaptive teacher blending')
...
class MedSAM2VolumeTeacher(torch.nn.Module):
    ...
    def _select_classes(self, prompt_probs):
        ...
        selected = []
        top_confident = max(1, class_candidates[0][0])
        for confident_voxels, _, class_id in class_candidates:
            if selected and (confident_voxels / top_confident) < float(args.medsam2_multi_class_ratio):
                continue
            selected.append(class_id)
            if len(selected) >= self.max_classes:
                break
        return selected
    ...
    def _pick_key_slices(self, class_prob, class_mask):
        ...
        elif self.num_condition_frames == 1:
            best_idx = torch.argmax(slice_area[valid_slices])
            selected = [int(valid_slices[best_idx].item())]
        else:
            # Prefer sparse anchors across the valid slice span instead of densely prompting
            # the top-k largest slices, which tends to over-condition SAM2.
            anchor_positions = torch.linspace(
                0, valid_slices.numel() - 1, steps=self.num_condition_frames, device=valid_slices.device
            ).round().long()
            selected = [int(valid_slices[idx].item()) for idx in anchor_positions]
            best_idx = int(valid_slices[torch.argmax(slice_area[valid_slices])].item())
            if best_idx not in selected:
                weakest_pos = min(range(len(selected)), key=lambda pos: float(slice_area[selected[pos]].item()))
                selected[weakest_pos] = best_idx
    ...
    def _add_prompts(self, inference_state, class_prompt_map):
        valid_prompt = False
        for class_id, prompt_item in class_prompt_map.items():
            prompt_mask = prompt_item["prompt_mask"]
            selected_slices = prompt_item["selected_slices"]
    ...
    def _propagate_single_direction(self, frame_sequence, class_prompt_map, reverse=False):
        ...
        if not self._add_prompts(state, class_prompt_map):
            self.predictor.reset_state(state)
            return None, []

        class_ids = sorted(int(class_id) for class_id in class_prompt_map.keys())
        class_index = {class_id: idx for idx, class_id in enumerate(class_ids)}
        class_scores = prompt_mask.new_zeros((len(class_ids), height, width, depth), dtype=torch.float32)
        all_prompt_slices = [slice_idx for item in class_prompt_map.values() for slice_idx in item["selected_slices"]]
        start_frame_idx = max(all_prompt_slices) if reverse else min(all_prompt_slices)
        for frame_idx, object_ids, out_mask_logits in self.predictor.propagate_in_video(
            state,
    ...
    def forward(self, images, prompt_probs):
        ...
        with autocast_context:
            for batch_idx in range(batch_size):
                ...
                scores_forward, class_ids_forward = self._propagate_single_direction(
                    frame_sequence, class_prompt_map, reverse=False
                )
                scores_reverse, class_ids_reverse = self._propagate_single_direction(
                    frame_sequence, class_prompt_map, reverse=True
                )
                ...
                if scores_forward is not None:
                    for score_idx, class_id in enumerate(class_ids_forward):
                        class_score_map[int(class_id)] = scores_forward[score_idx]
                if scores_reverse is not None:
                    for score_idx, class_id in enumerate(class_ids_reverse):
                        if int(class_id) in class_score_map:
                            if args.medsam2_direction_merge == 'max':
                                class_score_map[int(class_id)] = torch.maximum(
                                    class_score_map[int(class_id)], scores_reverse[score_idx]
                                )
                            else:
                                class_score_map[int(class_id)] = 0.5 * (
                                    class_score_map[int(class_id)] + scores_reverse[score_idx]
                                )
                        else:
                            class_score_map[int(class_id)] = scores_reverse[score_idx]
...
def get_adaptive_blend_alpha(base_alpha, trusted_ratio, agreement_ratio):
    if base_alpha <= 0:
        return 0.0
    if trusted_ratio < float(args.medsam2_blend_min_trust):
        return 0.0
    if agreement_ratio < float(args.medsam2_blend_min_agree):
        return 0.0
    trust_scale = min(1.0, trusted_ratio / max(float(args.medsam2_blend_min_trust), 1e-6))
    agree_scale = min(1.0, agreement_ratio / max(float(args.medsam2_blend_min_agree), 1e-6))
    return float(base_alpha) * trust_scale * agree_scale
...
def train(labeled_list, unlabeled_list, eval_list, fold_id=1):
    ...
    if args.resume_path:
        ...
        iter_num, best_dice_avg = load_resume_state(model, ema_model, optimizer, args.resume_path)
        logging.info(
                ...
                if (
                    ...
                    if medsam2_patch_probs is not None and medsam2_patch_coverage is not None:
                        ...
                        if bool(args.medsam2_main_teacher_blend):
                            blend_alpha = float(args.medsam2_blend_alpha)
                            if bool(args.medsam2_adaptive_blend):
                                blend_alpha = get_adaptive_blend_alpha(
                                    blend_alpha,
...
if __name__ == "__main__":
    ...
    shutil.copyfile(current_script, os.path.join(snapshot_path, "code", os.path.basename(current_script)))
    ...
    model.eval()
    _, _, metric_final = test_util.validation_all_case_btcv(model, num_classes=num_classes, base_dir=train_data_path, image_list=test_list, patch_size=patch_size, stride_xy=32, stride_z=16)
