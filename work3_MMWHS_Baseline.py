...
def train(labeled_list, unlabeled_list, eval_list, fold_id=1):
    ...
    db_train = MMWHS_fast(labeled_list, unlabeled_list,
...
if __name__ == "__main__":
    import setproctitle
    setproctitle.setproctitle(f"MMWHS_{args.labelnum}")
    import stat
    ...
    shutil.copyfile("./work3_MMWHS_Baseline.py", snapshot_path + "/code/work3_MMWHS_Baseline.py")  # 保存当前文件
...
CUDA_VISIBLE_DEVICES=1 python work3_MMWHS_Baseline.py \
  --labelnum 10 \
  --max_iteration 35000 \
  --labeled_bs 2 \
  --batch_size 4 \
  --use_medsam2 1 \
  --medsam2_num_classes 8 \
  --medsam2_warmup 3000 \
  --medsam2_interval 50 \
  ...
  --medsam2_min_voxels 500 \
  --medsam2_min_slice_area 80 \
